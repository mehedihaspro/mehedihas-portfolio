...
// The celebratory heading that anchors the share view.
const TIERS: ScoreTier[] = [
  {
    min: 100,
    headline: "Flawless!",
    subline: "You nailed every single question.",
  },
  {
    min: 80,
    headline: "You aced it.",
    subline: "Clearly read it end to end.",
  },
  {
    min: 60,
    headline: "Well done!",
    subline: "You got the main ideas.",
  },
  {
    min: 40,
    headline: "Not bad at all.",
    subline: "A few tricky ones got away.",
  },
  {
    min: 0,
    headline: "Give it another run.",
    subline: "The article has a few surprises.",
  },
];
...
export function QuizShareModal({
  ...
  // Monochrome social row — one consistent treatment across every tile.
  const socialLinks = [
    {
      name: "X",
    ...
    },
    {
      ...
      href: `mailto:?subject=${encodeURIComponent(
        `I scored ${score}/${total} on a mehedihas quiz`
  ...
  return (
    <QuizModalShell isOpen onDismiss={onClose} ariaLabel="Share your score">
      ...
      <div className="px-6 pb-5 text-center">
        <p className="text-[10px] font-mono uppercase tracking-[0.14em] text-amber mb-2">
          {percent}% score
        </p>
        <h2 className="font-display font-bold text-[40px] md:text-[52px] leading-[0.95] text-text-primary tracking-[-0.02em] mb-2">
          {tier.headline}
        </h2>
        <p className="text-[14px] text-text-secondary font-inter max-w-[360px] mx-auto">
          {tier.subline}
        </p>
      </div>

      {/* Score preview card — mirrors the OG image */}
      <div className="px-6 pb-5">
        <div className="rounded-[14px] border border-border bg-bg p-5">
          ...
          <div className="flex items-baseline gap-1.5 mb-3">
            <span className="text-[52px] font-bold leading-none text-text-primary tracking-[-0.03em] font-inter tabular-nums">
              {score}
            </span>
            <span className="text-[22px] font-bold text-text-muted font-inter tabular-nums">
              /{total}
            </span>
            <span className="ml-auto text-[12px] font-bold text-amber font-inter tabular-nums">
              {percent}%
            </span>
          </div>
      ...
      <div className="px-6 pb-3">
        <p className="text-[10px] font-mono uppercase tracking-[0.1em] text-text-muted mb-2">
          Share on
        </p>
      ...
      {/* Copy link — primary action, dark per the button system */}
